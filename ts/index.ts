...
import type { TClContext, TClDevice, TClPlatform } from './native.ts';
...
Object.defineProperty(Wrapper.prototype, inspect.custom, {
	value: Wrapper.prototype.toString,
	configurable: true,
});

const logger = getLogger('opencl');

type TDeviceCandidate = Readonly<{
...
type TQuickStartCache = Readonly<{
	...
	result: TQuickStartResult;
}>;

let cached: TQuickStartCache | null = null;

const showDevices = (devices: readonly TDeviceCandidate[], mainDevice: TDeviceCandidate): void => {
	logger.info(
		'AVAILABLE DEVICES:',
		devices.map(({ name, version }) => `${version} ${name}`),
	);
	logger.info('ACTIVE DEVICE:', `${mainDevice.version} ${mainDevice.name}`);
};

/**
 * Quickly grab a good CL context.
 *
 * Multiple calls return the same context. Pass true to log the intermediate device list.
 */
export const quickStart = (isLoggingDevices = false): TQuickStartResult => {
	if (cached) {
		if (isLoggingDevices) {
	...
	const devicesAll = getPlatformIDs().flatMap((platform): TDeviceCandidate[] => {
		const version = getPlatformInfo(platform, PLATFORM_VERSION);
		const platformDevices = getDeviceIDs(platform);
		return platformDevices.map((device): TDeviceCandidate => {
			const name = getDeviceInfo(device, DEVICE_NAME);
			const type = getDeviceInfo(device, DEVICE_TYPE);

			if (typeof name !== 'string' || typeof type !== 'number') {
				throw new TypeError('OpenCL device info returned an unexpected shape.');
			}
			...
			return {
				...
				name,
				type,
				label: `${type === DEVICE_TYPE_GPU ? 'GPU ' : ''}${version} ${name}`,
			};
		});
	});
	...
	if (!mainDevice) {
		throw new Error('No OpenCL devices found.');
	}
