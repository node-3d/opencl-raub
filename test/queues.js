			...
			function () {
				U.withContext(function (ctx, device) {
					U.withCQ(ctx, device, function (cq) {
						...
						readBound.should.throw(cl.INVALID_OPERATION.message);
						cl.releaseMemObject(image);
					});
				});
			}
		...
		it('should work with read only buffers', function () {
			U.withContext(function (ctx, device) {
				U.withCQ(ctx, device, function (cq) {
					...
					var ret = cl.enqueueCopyImageToBuffer(
						cq,
						image,
						buffer,
						[0, 0, 0],
		...
		it('should work with read only buffers', function () {
			U.withContext(function (ctx, device) {
				U.withCQ(ctx, device, function (cq) {
					...
					var ret = cl.enqueueCopyBufferToImage(
						cq,
						buffer,
						image,
						0,
